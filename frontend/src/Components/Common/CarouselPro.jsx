...
const Carousel = ({ slides = [] }) => {
  ...
  useEffect(() => {
    const handleMouseMove = (e) => {
      ...
      // Velocity smoothing for the RGB split
      setVelocity({ x: dx * 0.15, y: dy * 0.15 });
      setMousePos({ x: e.clientX, y: e.clientY });
      lastMousePos.current = { x: e.clientX, y: e.clientY };
    };
    const friction = setInterval(() => {
  ...
  return (
    ...
    >
        ...
        {adjacentSlides.map((slide, i) => {
          ...
          return (
            <div key={i} className="min-w-full h-full relative overflow-hidden flex-shrink-0">
              ...
              {/* TITLES WITH RGB SPLIT & PARALLAX */}
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                <div style={{
                  transform: `translateX(${(index - i) * 25}%) translate3d(${tiltX}px, ${tiltY}px, 0)`,
                  transition: 'transform 0.1s ease-out'
                }}>
                  <div className="relative inline-block">
                    {/* Cyan Ghost (Follows velocity) */}
                    <h2 className="absolute text-cyan-400 text-6xl md:text-9xl font-black uppercase italic tracking-tighter"
                      style={{ transform: `translate3d(${velocity.x}px, ${velocity.y}px, 0)` }}>
                      {slide.title}
                    </h2>
                    {/* Red Ghost (Opposite velocity) */}
                    <h2 className="absolute text-red-500 text-6xl md:text-9xl font-black uppercase italic tracking-tighter"
                      style={{ transform: `translate3d(${-velocity.x}px, ${-velocity.y}px, 0)` }}>
                      {slide.title}
                    </h2>
                    {/* Main White Layer */}
                    <h2 className="relative text-white text-6xl md:text-9xl font-black uppercase italic tracking-tighter drop-shadow-[0_15px_15px_rgba(0,0,0,0.8)]">
                      {slide.title}
                    </h2>
                  </div>
                </div>
      ...
      <div className={`absolute inset-0 flex items-center justify-between px-6 pointer-events-none transition-opacity duration-500 ${isPlaying ? "opacity-0" : "opacity-100"}`}>
        <button
          onClick={() => { handlePrev(); setIsPlaying(false); }}
          className="pointer-events-auto p-4 rounded-full bg-black/30 text-white backdrop-blur-sm hover:bg-white hover:text-black transition-all"
        >
          <ChevronLeft size={40} />
        </button>
        <button
